...
def getStockPrice(shortName, assetType):
    if assetType == "Bonds":
        url=f"https://iss.moex.com/iss/engines/stock/markets/bonds/securities/{shortName}.json"
        try:
            req = requests.get(url).json()
        except requests.exceptions.SSLError:
            time.sleep(1000)
            req = requests.get(url).json()
        data=req["marketdata"]["data"][-1][11]
        if data==None:
    ...
    if assetType == "Stocks":
        ...
        return float(data)
    if assetType == "Metals":
        url=f"https://iss.moex.com/iss/engines/currency/markets/selt/securities/{shortName}.json"
        try:
            req = requests.get(url).json()
        except requests.exceptions.SSLError:
            time.sleep(1000)
...
def totalCapital():
    ...
    for asssetType in ("Stocks", "Metals", "Bonds"):
        ...
        for stock in stocks:
            ...
            total_price_now=0
            price_now=getStockPrice(stock['stock'], asssetType) #цена акций сейчас
